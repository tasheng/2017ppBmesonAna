...
//	gStyle->SetOptTitle(0);
	...
	FileName = "OutFile/SkimmedCutBP2015.root";
	...
	EffInfoTree->SetBranchAddress("BEffInvCorrDiffHis2Err",BEffInvErr);
...
*/
	...
	TFile * fin2DEff = new TFile("NewEff2DMaps/EffFine.root");
	fin2DEff->cd();
	TH1D * invEff2D = (TH1D * ) fin2DEff->Get("invEff2D");
	...
	for( int i = 0; i < NEvents; i++){
		...
		for(int j = 0; j < BsizeNew; j++){
			...
			for(int k = 0; k < NBins; k++){
				...
				if(BptNew[j] > ptBins[k] && BptNew[j] < ptBins[k+1] && TMath::Abs(BmassNew[j] - 5.27932) < 0.08 && ByNew[j] < 2.4)
				{
					//	SumCountsReal[k] = SumCountsReal[k] + BEff[j];
					//	if(k == 0) cout << "Eff Inv Here  = " << BEffInv[j] << endl; 

				
					XPosInv = invEff2D->GetXaxis()->FindBin(BptNew[j]);
					YPosInv = invEff2D->GetYaxis()->FindBin(abs(ByNew[j]));
					
					BEffInv[j] =  invEff2D->GetBinContent(XPosInv,YPosInv);
					BEffInvErr[j] =  invEff2D->GetBinError(XPosInv,YPosInv);
...
*/
	...
	*/
	...
	TFile * RawYield = new TFile("RawYields/yields_Bp_binned_pt_2015.root");
	RawYield->cd();
	...
	TFile * foutCorr = new TFile("FinalFiles/BPPPCorrYieldPT2015.root","RECREATE");
	foutCorr->cd();
...
//	CorrDiffHis->SetTitle("2017 B^{+} pp Cross Section");
	...
	TFile * fin1DEff = new TFile("NewEff2DMaps/EffFine.root");
	fin1DEff->cd();
	TH1D * Eff1DHis = (TH1D * ) fin1DEff->Get("Eff1DHis");


	CorrDiffHisBin->SetMarkerColor(kBlue);
	CorrDiffHisBin->SetLineColor(kBlue);	
	...
	TFile * fin2017 = new TFile("FinalFiles/BPPPCorrYieldPT.root");
	fin2017->cd();

	TH1D *	CorrDiffHis2017 = (TH1D *) fin2017->Get("hPtSigma");
	CorrDiffHis2017->SetMarkerColor(kRed);
	CorrDiffHis2017->SetLineColor(kRed);

	CorrDiffHis2017->Draw("ePSAME");

	TLegend *leg = new TLegend(0.30,0.58,0.75,0.85,NULL,"brNDC");
	leg->SetBorderSize(0);
	...
	leg->AddEntry(CorrDiffHis2017,"2017 pp (2017 BDT Cuts)","pl");
	leg->AddEntry(CorrDiffHisBin,"2017 pp Eff Binned Corr (2015 Cuts)","pl");
	leg->AddEntry(CorrDiffHis,"2017 pp (2015 Cuts)","pl");
	...
	c->SaveAs("FinalPlots/CorrectedYield6BinsAll.png");
	...
	const int NPtBins2 = 5;
	float PtBin2[NPtBins2 + 1] = {7,10,15,20,30,50};


	TH1D * CorrRatio = new TH1D("CorrRatio","",NPtBins2,PtBin2);


	CorrRatio->GetXaxis()->SetTitle("B^{+} p_{T} (GeV/c)");
	CorrRatio->GetYaxis()->SetTitle("2015 pp / 2017 pp Cross Section Ratio (Scaled to 2017 Stat)");
	CorrRatio->SetTitle("2015 vs 2017 Dataset Cross Section Yield Ratio");
	CorrRatio->GetXaxis()->CenterTitle();
	CorrRatio->GetYaxis()->CenterTitle();
	CorrRatio->GetYaxis()->SetTitleOffset(1.4);

	

	CorrRatio->SetMarkerColor(kBlack);
	CorrRatio->SetLineColor(kBlack);
	CorrRatio->SetMarkerSize(1);
	CorrRatio->SetMarkerStyle(20);
	...
	for(int i = 0; i < 5; i ++){
		...
		CorrV2015 = BPXSecHis->GetBinContent(i+1);
		CorrErr2015 = BPXSecHis->GetBinError(i+1);

		CorrV2017 = CorrDiffHisBin->GetBinContent(i+BinMin);
		CorrErr2017 = CorrDiffHisBin->GetBinError(i+BinMin);

		Ratio = CorrV2017/CorrV2015;
	
		cout << "Ratio = " << Ratio << endl;
	
		RatioErr = Ratio * sqrt((CorrErr2015/CorrV2015) * (CorrErr2015/CorrV2015) + (CorrErr2017/CorrV2017) * (CorrErr2017/CorrV2017));

		CorrRatio->SetBinContent(i+1,Ratio);
		CorrRatio->SetBinError(i+1,RatioErr);

	}

	TLine *line = new TLine(7,1,50,1);
	line->SetLineStyle(2);
	line->SetLineWidth(2);
	line->SetLineColor(2);
	

	CorrRatio->SetMaximum(1.2);
	CorrRatio->SetMinimum(0.6);

	CorrRatio->Draw("ep");
	line->Draw("SAME");

	c2->SaveAs("FinalPlots/CorrRatio.png");
