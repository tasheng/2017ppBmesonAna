...
void  ManualTestBDTPrivate(){
	...
	TString infile = "/data/szhaozho/Bmeson2017pp/Private/BPMCPrivate.root";
...
*/
	...
	const int NPtBins = 7;
	double PtBin[NPtBins + 1] = {3,5,7,10,15,20,50,100};
...
//	TH1D * Eff1DGENHis = new TH1D("Eff1DGENHis","",NPtBins,PtBin);
	...
	TString outfileName = "NewEff2DMaps/EffFineBDTPrivate.root";
	...
	for(int i = 0; i < NEvents; i++){
		...
		TFile * fout2 = new TFile(Form("BPMuonInfoPlots_%d_%dPrivate.root",ptmin,ptmax),"RECREATE");
		fout2->cd();
		...
		invAcc2D->GetXaxis()->SetTitle("B_{s} p_{T} (GeV/c)");
		invAcc2D->GetXaxis()->CenterTitle();
		invAcc2D->GetYaxis()->CenterTitle();
		invAcc2D->GetYaxis()->SetTitleOffset(1.2);
		invAcc2D->SetTitle("");
		c->SaveAs("Eff2DMapTnP/Eff2D.png");
