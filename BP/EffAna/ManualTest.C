...
//	TString infile = "/export/d00/scratch/zzshi/CMSSW_7_5_8_patch3/Merge/ppDataSets/MC/BPlus/BPMCAllNoCut.root";
	TString infile = "/data/szhaozho/Bmeson2017pp/BDTOutput/AllMerge/BPMCAllBDT.root";
...
*/
	...
	const int NPtBins = 4;
	double PtBin[NPtBins + 1] = {7,10,15,20,50};

	const int NPtBins1D = 8;
	double  PtBin1D[NPtBins1D + 1] = {3,5,7,10,15,20,30,50,100};
	...
	Eff1DRECOHis->GetYaxis()->SetTitle("#alpha #times #episolon");
...
//	TH1D * Eff1DGENHis = new TH1D("Eff1DGENHis","",NPtBins,PtBin);
	...
	Eff1DGENHis->GetYaxis()->SetTitle("#alpha #times #episolon");
	...
	TString outfileName = "NewEff2DMaps/EffFine.root";
	...
	for(int i = 0; i < NEvents; i++){
		...
		for(int j = 0; j < Bsize; j++){
			...
			if( (HLT_HIL1DoubleMu0_v1 == 1) && (Bmu1isTriggered[j] == 1 && Bmu2isTriggered[j] == 1 ) &&  (HBHENoiseFilterResult == 1 && pPAprimaryVertexFilter == 1 && pBeamScrapingFilter == 1) &&  (abs(PVz)<15&&TMath::Abs(By[j])<2.4&&TMath::Abs(Bmumumass[j]-3.096916)<0.15&&Bmass[j]>5&&Bmass[j]<6&& ((abs(Bmu1eta[j])<1.2 && Bmu1pt[j]>3.5) || (abs(Bmu1eta[j])>1.2 && abs(Bmu1eta[j])<2.1 && Bmu1pt[j]>(5.77-1.8*abs(Bmu1eta[j]))) || (abs(Bmu1eta[j])>2.1 && abs(Bmu1eta[j])<2.4 && Bmu1pt[j]>1.8)) && ((abs(Bmu2eta[j])<1.2 && Bmu2pt[j]>3.5) || (abs(Bmu2eta[j])>1.2 && abs(Bmu2eta[j])<2.1 && Bmu2pt[j]>(5.77-1.8*abs(Bmu2eta[j]))) || (abs(Bmu2eta[j])>2.1 && abs(Bmu2eta[j])<2.4 && Bmu2pt[j]>1.8)) && Bmu1TMOneStationTight[j] && Bmu2TMOneStationTight[j] && Bmu1InPixelLayer[j] > 0 && (Bmu1InPixelLayer[j]+Bmu1InStripLayer[j]) > 5 && Bmu2InPixelLayer[j] > 0 && (Bmu2InPixelLayer[j]+Bmu2InStripLayer[j]) > 5 && Bmu1dxyPV[j]< 0.3 && Bmu2dxyPV[j]< 0.3 && Bmu1dzPV[j]<20 && Bmu2dzPV[j]<20 && Bmu1isGlobalMuon[j] && Bmu2isGlobalMuon[j]  && Btrk1highPurity[j] && abs(Btrk1Eta[j])<2.4 && Btrk1Pt[j]>0.5 && Bchi2cl[j]>0.005 && ((Bpt[j]<10 && (BsvpvDistance[j]/BsvpvDisErr[j])>5.5) || (Bpt[j]>10 && (BsvpvDistance[j]/BsvpvDisErr[j])>3.5)) && ((Bpt[j]>7 && Bpt[j]<10 && Btrk1Pt[j]>0.748 && Bchi2cl[j]>0.0322 && (BsvpvDistance[j]/BsvpvDisErr[j])>0 && cos(Bdtheta[j])>0.989 && abs(Btrk1Eta[j])<2.4)|| (Bpt[j]>10 && Bpt[j]<15 && Btrk1Pt[j]>0.878 && Bchi2cl[j]>0.0014 && (BsvpvDistance[j]/BsvpvDisErr[j])>0 && cos(Bdtheta[j])>0.971 && abs(Btrk1Eta[j])<2.39) || (Bpt[j]>15 && Bpt[j]<20 && Btrk1Pt[j]>0.818 && Bchi2cl[j]>0.00838 && (BsvpvDistance[j]/BsvpvDisErr[j])>0 && cos(Bdtheta[j])>0.959 && abs(Btrk1Eta[j])<2.42) || (Bpt[j]>20 && Bpt[j]<30 && Btrk1Pt[j]>0.84 && Bchi2cl[j]>0.014 && (BsvpvDistance[j]/BsvpvDisErr[j])>0 && cos(Bdtheta[j])>0.603 && abs(Btrk1Eta[j])<2.38) || (Bpt[j]>30 && Bpt[j]<50 && Btrk1Pt[j]>1.06 && Bchi2cl[j]>0.0155 && (BsvpvDistance[j]/BsvpvDisErr[j])>3.53 && cos(Bdtheta[j])>0.995 && abs(Btrk1Eta[j])<2.37) || (Bpt[j]>50) || (Bpt[j]>5 && Bpt[j]<7 && Btrk1Pt[j]>0.748 && Bchi2cl[j]>0.0322 && (BsvpvDistance[j]/BsvpvDisErr[j])>0 && cos(Bdtheta[j])>0.989 && abs(Btrk1Eta[j])<2.4) ||(Bpt[j]>0 && Bpt[j]<5 && (BsvpvDistance[j]/BsvpvDisErr[j])>17) )) && Bgen[j] == 23333 ){
				...
				//cout << "RECO: EventWeight = " << EventWeight  << endl;
		...
		for(int i = 0; i < NEvents; i++){
			...
			//cout << "GEN: EventWeight = " << EventWeight  << endl;
		...
		for(int i = 0; i < NPtBins1D; i ++){
			...
			cout << "pT =  " << PtBin1D[i] << " - " << PtBin1D[i+1] << "  Eff:  " << Eff1DHis->GetBinContent(i+1)   << "   Eff Error: " << Eff1DHis->GetBinError(i+1)<< endl;
