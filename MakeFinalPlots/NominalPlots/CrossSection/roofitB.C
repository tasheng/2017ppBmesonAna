...
//	 std::vector<std::string> signal = {"3gauss"};
	...
	//      return;
		RooDataSet* ds_cut;
		if(doubly==0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10)) && HiBin>=%f&&HiBin<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto,hiBinMin,hiBinMax)); 
		if(doubly==1)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&HiBin>=0&&HiBin<=60&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		if(doubly==2)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&HiBin>=60&&HiBin<=180&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 

	    //  RooRealVar* w = (RooRealVar*) data->addColumn(wFunc) ;
	//      RooDataSet wdata(data->GetName(),data->GetTitle(),data,*data->get(),0,w->GetName()) ;
				//dsMC = new RooDataSet(Form("dsMC%d",_count),"",skimtreeMC_new,RooArgSet(*mass, *pt));
		RooDataSet* dsMC_cut;
		if(doubly==0) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10))&& HiBin>=%f&&HiBin<=%f",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto,hiBinMin,hiBinMax), "Pthatweight"); 
		if(doubly==1) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&HiBin>=0&&HiBin<=60&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "Pthatweight"); 
		if(doubly==2) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *HiBin, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&HiBin>=60&&HiBin<=180&&((Bpt<10&&Bpt>5&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "Pthatweight"); 
	     // return;
		...
		std::ofstream Summary("Summary.dat",ios::app);
	
		RooRealVar* fitMean = static_cast<RooRealVar*>(f->floatParsFinal().at(f->floatParsFinal().index(Form("mean%d",_count))));


		Summary <<  "Mean =    " << fitMean->getVal()   <<    "   Mean Error "  << fitMean->getError() << "   yield = " << yield << "  yieldErr =   " << yieldErr     <<  "   Background = " <<  bkgd << "  Background Err =   "   << bkgdErr  << endl;
...
//		if(drawOpt==1){
		...
		if(ptBins_check[i+1] < 10.1){
			tex_y = new TLatex(0.21,0.69,"1.5 < |y| < 2.4");
