...
RooFitResult *fit(TString variation, TString pdf,TString tree, TCanvas* c, TCanvas* cMC, RooDataSet* ds, RooDataSet* dsMC, RooDataHist* dh, RooDataHist* dhMC, RooRealVar* mass, RooPlot* &outframe, Double_t ptmin, Double_t ptmax, int isMC, bool isPbPb, Float_t centmin, Float_t centmax, TString npfit)
{
	...
	/*
	   ...
	   if(ptmin == 7) MaxFrame = 150;
	   if(ptmin == 15) MaxFrame = 600;
	   if(ptmin == 20) MaxFrame = 600;
	   if(ptmin == 10 && ptmax == 50 && centmin == 0 && centmax ==90) MaxFrame = 4000;
	   if(ptmin == 10 && ptmax == 50 && centmin == 0 && centmax ==30) MaxFrame = 1300;
	   if(ptmin == 10 && ptmax == 50 && centmin == 30 && centmax ==90) MaxFrame = 1200;

	   if(ptmin == 10 && ptmax == 15) MaxFrame = 1200;


	   if(ptmin == 7) MaxYield = 200;
	   if(ptmin == 15) MaxYield = 700;
	   if(ptmin == 20) MaxYield = 700;
	   if(ptmin == 10 && ptmax == 50 && centmin == 0 && centmax ==90) MaxYield = 2000;
	   if(ptmin == 10 && ptmax == 50 && centmin == 0 && centmax ==30) MaxYield = 1500;
	   if(ptmin == 10 && ptmax == 50 && centmin == 30 && centmax ==90) MaxYield = 800;

	   if(ptmin == 10 && ptmax == 15) MaxYield = 700;
...
//	ds->plotOn(frame, RooFit::XErrorSize(0));//draw an transparent hist

	ds->plotOn(frame,Name(Form("ds_cut%d",_count)),Binning(nbinsmasshisto),MarkerSize(1),MarkerStyle(20),MarkerColor(1),LineColor(1),LineWidth(4), RooFit::XErrorSize(0));//draw an transparent hist
	...
	//   if(tree=="ntKp")frame->SetMaximum(nsig.getVal()*0.47);
	//	if(tree=="ntKp")frame->SetMaximum(nsig.getVal()*0.90);

	if(tree=="ntKp")frame->SetMaximum(nsig.getVal()*0.7);

//	frame->SetMaximum(450);
	...
	std::ofstream Chi2Value("Chi2Out.txt",ios::app);
	Chi2Value << ptmin << "   " << ptmax << "   " << frame->chiSquare(Form("model%d",_count),Form("ds_cut%d",_count), fitResult->floatParsFinal().getSize()) << "  " << fitResult->floatParsFinal().getSize() << endl;
	...
	Double_t NPBkgd = npeakbg.getVal();

	cout << "==================================================================================== " << endl;


	cout << "NPBkgd = " << NPBkgd << endl;
	...
	TLatex* texB;
	if(tree=="ntphi") texB = new TLatex(0.21+0.013,0.82,"B^{0}_{s}");
	//  if(tree=="ntKp") texB = new TLatex(0.21,0.8,"B^{#pm}");
	if(tree=="ntKp") texB = new TLatex(0.21+0.013,0.82,"B^{+}");
	if(drawSup) texB = new TLatex(0.55,0.84,"B^{0}_{s}");
	texB->SetNDC();
	texB->SetTextFont(62);
	texB->SetTextSize(0.065*1.15);
	texB->SetLineWidth(2);
	...
	bkgdErr = nbkg.getError()*bkgIntegral->getVal();
	...
	frame->GetYaxis()->SetTitleSize(frame->GetYaxis()->GetTitleSize() * 1.4);
	frame->Draw();
	...
	pull_plot->GetYaxis()->SetTitleOffset(0.4);
	pull_plot->SetYTitle("Pull");
	pull_plot->GetXaxis()->SetTitleSize(pull_plot->GetXaxis()->GetTitleSize() * 1.3);
	pull_plot->GetYaxis()->SetTitleSize(pull_plot->GetYaxis()->GetTitleSize() * 1.15);
	//	pull_plot->GetYaxis()->SetTitleSize(YSize);
	pull_plot->GetYaxis()->SetTitleOffset(0.33);


	pull_plot->GetXaxis()->SetTitleOffset(0.8);
...
//	gPad->SetBottomMargin(0.05);
	...
	if(tree=="ntKp"){
		...
		c_pull->SaveAs(Form("new/pdfForGJ/pull_signal_full_%d_%d_%d.pdf",index,CentMin,CentMax));
		c_params->SaveAs(Form("new/pdfForGJ/params_signal_full_%d_%d_%d.pdf",index,CentMin,CentMax));
		c_errors->SaveAs(Form("new/pdfForGJ/error_signal_full_%d_%d_%d.pdf",index,CentMin,CentMax));
