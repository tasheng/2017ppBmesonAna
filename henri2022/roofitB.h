...
RooFitResult *fit(TString variation, TString pdf,TString tree, TCanvas* c, TCanvas* cMC, RooDataSet* ds, RooDataSet* dsMC, RooDataHist* dh, RooDataHist* dhMC, RooRealVar* mass, RooPlot* &outframe, Double_t ptmin, Double_t ptmax, int isMC, TString npfit)
{
	...
	RooRealVar sigma1MC(Form("sigma1MC%d_%s",_count,pdf.Data()),"",0.02,0.01,0.5) ;
	RooRealVar sigma2MC(Form("sigma2MC%d_%s",_count, pdf.Data()),"",0.055,0.01,0.5) ;
	RooRealVar sigma3MC(Form("sigma3MC%d_%s",_count, pdf.Data()),"",0.0266,0.01,0.5) ;
	RooRealVar sigma4cbMC(Form("sigma4cbMC%d_%s",_count, pdf.Data()),"",0.02,0.01,0.2) ;
	RooRealVar sigma5cbMC(Form("sigma5cbMC%d_%s",_count, pdf.Data()),"",0.02,0.01,0.2) ;
	...
	RooAddPdf* sigMC;
	RooRealVar nsigMC(Form("nsigMC%d_%s",_count, pdf.Data()),"",1, 0, 1.2 * dsMC->sumEntries());
...
///////////////// BACKGROUND FUNCTIONS
	...
	RooRealVar lambda(Form("lambda%d", _count), "lambda",-0.4, -1., .5);
