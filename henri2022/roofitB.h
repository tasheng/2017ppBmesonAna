...
if(tree=="ntphi"){
	if (varExp=="Bpt"){
		...
		double lolimit2[13]={5.35,0.005,0.01,0.005,0.01,0.01,0,0,0,0,0.001,0.001,0};
		double hilimit2[13]={5.38,0.1,0.06,0.02,0.1,0.1,20,50,500,500,.999,.999,2};
		for (int i=0;i<13;i++){
			init[i]=init2[i];
	...
	if (varExp=="Bpt"){
		...
		double lolimit2[13]={init_mean*0.9999,0.01,0.005,0.005,0.01,0.01,0,0,0,0,0.001,0.001,0.0};
		double hilimit2[13]={init_mean*1.00001,0.11,0.06,0.025,0.1,0.1,20,50,250,500,.99,.99,2.0};
		for (int i=0;i<13;i++){
			init[i]=init2[i];
			lolimit[i]=lolimit2[i];
			hilimit[i]=hilimit2[i];
		...
		RooRealVar sigma3MC(Form("sigma3MC%d_%s",_count, pdf.Data()),"",init[3],lolimit[3],hilimit[3]) ;
		RooRealVar sigma4cbMC(Form("sigma4cbMC%d_%s",_count, pdf.Data()),"",init[4],lolimit[4],hilimit[4]) ;
		RooRealVar sigma5cbMC(Form("sigma5cbMC%d_%s",_count, pdf.Data()),"",init[5],lolimit[5],hilimit[5]) ;
		RooRealVar alphaMC(Form("alphaMC%d_%s",_count,pdf.Data()),"",init[6],lolimit[6],hilimit[6]);
		RooRealVar alphaMC1(Form("alphaMC1%d_%s",_count,pdf.Data()),"",init[7],lolimit[7],hilimit[7]);
		RooRealVar nMC(Form("nMC_%d_%s", _count, pdf.Data()),"",init[8],lolimit[8],hilimit[8]);
		...
		RooRealVar sig1fracMC(Form("sig1fracMC%d_%s",_count, pdf.Data()),"",init[10],lolimit[10],hilimit[10]);
	...
	*/
	...
	//if((variation=="signal" && pdf=="2cb")) sigMC = new RooAddPdf(Form("sigMC%d_%s",_count,pdf.Data()),"",RooArgList(CBMC,CBMC1),sig1fracMC);
...
//////////ROOFIT ROOFIT ROOFIT  MC MC MC MC 
	...
	mass->setRange("signal",init_mean-0.2, init_mean+0.2);
...
*/
	...
	//frameMC->GetXaxis()->SetRangeUser(init_mean*0.99,init_mean*1.02);
	//frameMC->GetYaxis()->SetRangeUser(0,dsMC->sumEntries()*0.14);
	...
	RooRealVar n1(Form("n1_%d_%s", _count, pdf.Data()),"",nMC1.getVal(),0,500);
	...
	RooProduct scaled_sigma5cb("scaled_sigma5cb","scaled_sigma5cb", RooArgList(*scale,sigma5cb));
	...
	RooCBShape  CB1(Form("CB1%d_%s",_count, pdf.Data()),"",*mass,mean,scaled_sigma5cb, alpha1, n1);
	...
	*/
	...
	//if((variation=="signal" && pdf=="2cb") || variation== "background" || (variation=="signal" && pdf=="fixed")) sig = new RooAddPdf(Form("sig%d",_count),"",RooArgList(CB,CB1),sig1frac);
...
///////////////// BACKGROUND FUNCTIONS
	...
	// npfit is defined on the .sh files, is the model for B+->JPsi pi
...
*/
 ...
 ds->plotOn(frame, Name(Form("ds_cut%d", _count)), Binning(nbinsmasshisto/*,5.3,5.5*/),
...
void validate_fit(RooWorkspace* w, TString tree, TString variable, int full, int CentMin, int CentMax, int ptMin, int ptMax)
{
	...
	std::cout << "sigma1 is " << sigma1->getVal() << std::endl; 
