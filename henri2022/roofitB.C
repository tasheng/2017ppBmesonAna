...
//Resolution 
	...
	if(varExp=="Bpt"){
      ...
      if (drawLegend) {
        ...
        tex_pt = new TLatex(0.21,0.7,Form("%d < p_{T} < %d GeV/c",(int)_ptBins[i],(int)_ptBins[i+1]));
...
if(varExp=="nMult"){ 
	//for the paper run these
      if (drawLegend) {
		...
		tex_pt->SetTextSize(0.029);
		tex_pt->SetLineWidth(2);
		tex_pt->Draw();
		tex_nMult->SetNDC();
		tex_nMult->SetTextFont(42);
		tex_nMult->SetTextSize(0.029);
		...
		chi_square->SetTextSize(0.029);
		chi_square->SetLineWidth(2);
		chi_square->Draw();
		yield_val->SetNDC();
		yield_val->SetTextFont(42);
		yield_val->SetTextSize(0.029);
	...
	if(tree=="ntKp"){ texB = new TLatex(0.22,0.83, "B^{#pm}");}
	texB->SetNDC();
	texB->SetTextFont(62);
